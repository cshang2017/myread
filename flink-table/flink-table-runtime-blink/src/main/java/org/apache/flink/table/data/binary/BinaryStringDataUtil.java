...
public class BinaryStringDataUtil {
	...
	private static DecimalData toDecimalFromBytes(
		...
		// Data in DecimalData is stored by one long value if `precision` <= DecimalData.MAX_LONG_DIGITS.
		...
		while (i < sizeInBytes) {
			...
			if (b >= '0' && b <= '9') {
				// No need to worry about overflow, because sizeInBytes <= DecimalData.MAX_LONG_DIGITS
		...
		if (change >= 0) {
			...
			int k = negative ? -5 : 5;
			significand = (significand + k * DecimalDataUtils.power10(-change - 1)) / DecimalDataUtils.power10(-change);
		}
	...
	private static DecimalData toBigPrecisionDecimal(BinaryStringData str, int precision, int scale) {
		// As data in DecimalData is currently stored by BigDecimal if `precision` > DecimalData.MAX_LONG_DIGITS,
	...
	private static BinaryStringData keyValueSlow(
		...
		while (byteIdx < str.getSizeInBytes()) {
			...
			if (str.byteAt(byteIdx) == split1) {
				int currentKeyIdx = lastSplit1Idx + 1;
				BinaryStringData value = findValueOfKeySlow(str, split2, keyName, currentKeyIdx, byteIdx);
				if (value != null) {
					return value;
				}
				lastSplit1Idx = byteIdx;
	...
	private static BinaryStringData trimLeftSlow(BinaryStringData str, BinaryStringData trimStr) {
		...
		while (searchIdx < str.getSizeInBytes()) {
			...
			BinaryStringData currentChar = str.copyBinaryString(searchIdx, searchIdx + charBytes - 1);
			if (trimStr.contains(currentChar)) {
				searchIdx += charBytes;
	...
	public static BinaryStringData trimRight(BinaryStringData str, BinaryStringData trimStr) {
		...
		if (str.inFirstSegment()) {
			...
			charIdx -= 1;
			while (charIdx >= 0) {
				BinaryStringData currentChar = str.copyBinaryStringInOneSeg(
	...
	private static BinaryStringData trimRightSlow(BinaryStringData str, BinaryStringData trimStr) {
		...
		charIdx -= 1;
		while (charIdx >= 0) {
			BinaryStringData currentChar = str.copyBinaryString(
